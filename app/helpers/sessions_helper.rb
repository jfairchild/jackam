...
module SessionsHelper
  ...
  GO_OFF_KINGS_ID = '169164648'

  def self.twitch_auth_url(state_token)
    tok = Base64.urlsafe_encode64(state_token)
    "#{TWITCH_AUTH_BASE}oauth2/authorize?client_id=#{ENV['TWITCH_CLIENT_ID']}&redirect_uri=#{REDIRECT_URL}&response_type=code&#{TWITCH_SCOPES}&state=#{tok}"
  end

  def self.twitch_auth_user(twitch_code)
    "#{TWITCH_AUTH_BASE}oauth2/token?client_id=#{ENV['TWITCH_CLIENT_ID']}&client_secret=#{ENV['TWITCH_CLIENT_SECRET']}&code=#{twitch_code}&grant_type=authorization_code&redirect_uri=#{REDIRECT_URL}"
  end
